...
A face unlocks the right to attempt a prediction, a voice confirms it, then a product is
recommended. Dependencies are passed in so the whole transaction is testable without real files.
...
class AuthApp:
    ...
    def run_transaction(
        ...
        try:
            ...
            return self._finish(
                Decision(False, Stage.FACE, "no face could be detected in the image"), echo
            )
        ...
        try:
            ...
            return self._finish(
                Decision(False, Stage.VOICE, "clip too short to verify", identity=face.identity),
                echo,
            )
    ...
    @staticmethod
    def _finish(decision: Decision, echo: Echo) -> Decision:
        echo("")
        echo("-" * 58)
        if decision.granted:
